Role('Scope.Provider.Role.WithDOM', {
    ...
    requires    : [ 'getDocument', 'create', 'getPreload', 'isSetup' ],
    ...
    methods : {
        ...
        addOnErrorHandler : function (handler, callback) {
            ...
            var attachToOnError = function (scopeId) {
                
                var prevHandler         = window.onerror
                if (prevHandler && prevHandler.__MANAGED__) return
                
                window.onerror = function (message, url, lineNumber) {
                    var handler = (window.opener || window.parent).Scope.Provider.__ONERROR__[ scopeId ]
                ...
                window.onerror.__MANAGED__ = true
            }.toString()
            
            attachToOnError = ';(' + attachToOnError + ')(' + scopeId + ');'
            
            if (this.isSetup()) 
                this.runCode(attachToOnError, callback)
            else {
        ...
        setup : function (callback) {
            var isIE                = 'v' == '\v' || this.parentWindow.msPerformance
            var hasInlineScript     = false
            
            Joose.A.each(this.getPreload(), function (preloadDesc) {
                
                if (preloadDesc.type == 'js' && preloadDesc.content) {
            ...
            if (this.sourceURL || isIE && hasInlineScript) 
                ...
                this.setupWithDocWrite(callback)
        },
        
        
        setupWithDocWrite : function (callback) {
            var html        = []
            var me          = this
