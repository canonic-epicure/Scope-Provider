        ...
        addOnErrorHandler : function (handler, callback) {
            ...
            var attachToOnError = function (scopeId) {
                ...
                window.onerror = function (message, url, lineNumber) {
                    ...
                    // prevent recursive calls if other authors politely did not overwrite the handler
                    if (handler.__CALLED__) return
                    
                    handler.__CALLED__ = true
                    ...
                    setTimeout(function () {
                        handler.__CALLED__ = false
                    }, 0)
                    
                    return window.WebKitPoint ? false : true 
                }
                
                window.onerror.__FROM_TEST_RUN__ = true
            }
            
            if (this.scope) 
                this.runCode(';(' + attachToOnError.toString() + ')(' + scopeId + ');', callback)
            else {
                
                this.addPreload({
                    type        : 'js',
                    content     : ';(' + attachToOnError.toString() + ')(' + scopeId + ');'
                })
