...
//            var isOpera             = Object.prototype.toString.call(this.parentWindow.opera) == '[object Opera]'
        ...
        setupWithDocWrite : function (callback, needToSeed) {
            ...
            // cache the previous `onerror` handler
            var scope               = this.scope
            var prevOnError         = scope.onerror
            ...
            // `onerror` will survive the "doc.open()/close()" only in FF
            if (!scope.onerror && prevOnError) scope.onerror = prevOnError 
            
            console.log('Onerror - after doc close: ' + (me.scope.onerror != null))
        },
